"""
精细化CEV计算器 v2.5
实现基于v2.5论文的战斗效能值计算器，增加人口税和指挥官经济模型
...
class CalculationConfig:
    ...
    # 指挥官矿气转换率
    commander_mineral_gas_ratio: Dict[str, float] = None
    
    # 人口税豁免指挥官
    commanders_exempt_from_supply_tax: List[str] = None
    
    def __post_init__(self):
        """初始化默认值"""
        if self.splash_factors is None:
            self.splash_factors = {
                "SiegeTank": 1.4,        # 经v2.5调整的溅射效果
                "Liberator_AA": 1.8,     # 经v2.5调整的解放者AA模式AOE
        ...
        if self.commander_mineral_gas_ratio is None:
            # v2.5: 指挥官专属矿气转换率
            self.commander_mineral_gas_ratio = {
                "Nova": 1.5,        # 瓦斯采集效率高
                "Swann": 1.875,     # 瓦斯采集效率低
                "Alarak": 2.2,      # 献祭消耗额外矿物
                "AlarakP1": 2.2,    # 灵魂巧匠也使用2.2
                "Dehaka": 2.0,      # 标准
                "Artanis": 2.0,     # 标准
                "default": 2.0      # 默认值
            }
            
        if self.commanders_exempt_from_supply_tax is None:
            # v2.5: 豁免人口税的指挥官
            self.commanders_exempt_from_supply_tax = [
                "Nova",     # 幽灵兵营提供免费人口
                "Dehaka",   # 不需要补给建筑
            ]


class CEVCalculatorV25:
    """v2.5精细化CEV计算器"""
    ...
    def calculate_cev(self, unit_id: str, weapon_mode: Optional[str] = None,
        ...
        # 计算最终CEV
        # CEV (资源效率)
        cev = (dps_eff * psi * ehp * omega * f_range) / c_eff * mu
        
        # CEV_per_pop (人口效率)
        cev_per_pop = cev / unit_copy.supply if unit_copy.supply > 0 else 0
        ...
        return {
            ...
            "details": {
                ...
                # v2.5: 添加经济相关详情
                "population_tax": self._calculate_population_tax(unit_copy, commander),
                "mineral_gas_ratio": self._get_commander_mineral_gas_ratio(commander)
            }
        }
    ...
    def _calculate_effective_cost(self, unit: UnitData, commander: CommanderData) -> float:
        ...
        # v2.5: 使用指挥官专属矿气转换率
        alpha = self._get_commander_mineral_gas_ratio(commander)
        base_cost = minerals + vespene * alpha

        # v2.5: 计算人口税
        population_tax = self._calculate_population_tax(unit, commander)
        
        return base_cost + population_tax
    
    def _calculate_population_tax(self, unit: UnitData, commander: CommanderData) -> float:
        """
        计算人口税
        对于需要建造补给建筑的指挥官，每个人口消耗12.5矿物
        诺娃、德哈卡等指挥官豁免人口税
        """
        # 检查指挥官是否豁免人口税
        if commander.id in self.config.commanders_exempt_from_supply_tax:
            return 0
            
        # 计算人口税
        return unit.supply * self.config.population_tax_per_supply
        
    def _get_commander_mineral_gas_ratio(self, commander: CommanderData) -> float:
        """
        获取指挥官专属矿气转换率
        """
        return self.config.commander_mineral_gas_ratio.get(
            commander.id, self.config.commander_mineral_gas_ratio["default"])
...
if __name__ == "__main__":
    ...
    # 创建计算器
    calculator = CEVCalculatorV25(loader)
    
    # 测试计算v2.5模型下的单位CEV
    try:
        # 灵魂巧匠天罚行者
        result_wrathwalker = calculator.calculate_cev("Wrathwalker_AlarakP1", apply_mastery=True)
        print(f"\n灵魂巧匠天罚行者:")
        print(f"CEV: {result_wrathwalker['cev']}")
        print(f"CEV_per_pop: {result_wrathwalker['cev_per_pop']}")
        print(f"详细参数: {result_wrathwalker['components']}")
        print(f"经济详情: 矿气比率={result_wrathwalker['details']['mineral_gas_ratio']}, 人口税={result_wrathwalker['details']['population_tax']}")
        
        # 攻城坦克（对重甲）
        result_tank = calculator.calculate_cev("SiegeTankSieged_Swann", scenario="vs_armored", apply_mastery=True)
        print(f"\n攻城坦克（对重甲）:")
        print(f"CEV: {result_tank['cev']}")
        print(f"CEV_per_pop: {result_tank['cev_per_pop']}")
        print(f"详细参数: {result_tank['components']}")
        print(f"经济详情: 矿气比率={result_tank['details']['mineral_gas_ratio']}, 人口税={result_tank['details']['population_tax']}")
        
        # 掠袭解放者
        result_liberator = calculator.calculate_cev("Liberator_BlackOps", "AA", apply_mastery=True)
        print(f"\n掠袭解放者:")
        print(f"CEV: {result_liberator['cev']}")
        print(f"CEV_per_pop: {result_liberator['cev_per_pop']}")
        print(f"详细参数: {result_liberator['components']}")
        print(f"经济详情: 矿气比率={result_liberator['details']['mineral_gas_ratio']}, 人口税={result_liberator['details']['population_tax']}")
